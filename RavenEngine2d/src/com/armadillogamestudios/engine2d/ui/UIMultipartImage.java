...
public class UIMultipartImage<S extends Scene> extends UIImage<S> {
    ...
    public void draw(LayerShader shader) {
        ...
        shader.draw(getTexture(), spriteAnimationTop, getWorldPosition(), offset, null, getID(), getWorldZ(), getFade(), getHighlight(), DrawStyle.UI);


        for (int i = 0; i < rows; i++) {
            offset.y -= spriteAnimationMid.getHeight();
            shader.draw(getTexture(), spriteAnimationMid, getWorldPosition(), offset, null, getID(), getWorldZ(), getFade(), getHighlight(), DrawStyle.UI);

        }
        offset.y -= spriteAnimationBottom.getHeight();
        shader.draw(getTexture(), spriteAnimationBottom, getWorldPosition(), offset, null, getID(), getWorldZ(), getFade(), getHighlight(), DrawStyle.UI);
    }
