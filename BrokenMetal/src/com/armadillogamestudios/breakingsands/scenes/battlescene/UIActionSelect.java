...
import com.armadillogamestudios.engine2d.ui.container.UIBottomCenterContainer;
import com.armadillogamestudios.engine2d.ui.UIButton;
    ...
    public void setPawn(Pawn pawn) {
        ...
        btnEnd.setDisable(
                getScene().getActiveTeam() != 0 ||
                        getScene().getState() == BattleScene.State.MOVING ||
                        getScene().getState() == BattleScene.State.ATTACKING);
                ...
                if (ability.target == Ability.Target.SELF) {
                    ...
                    if (!isDisabled()) {
                        if (isActive()) {
                            setActive(false);
                            getScene().setActiveAbility(null);
                            getScene().setState(BattleScene.State.SELECT_DEFAULT);
                        } else {
                            btns.forEach(b -> b.setActive(false));
                            setActive(true);
                            getScene().setActiveAbility(getAbility());
