...
public class ModelReference {
	...
	public static void load(ModelData modelData) {
		ModelFrames modelFrames = new ModelFrames();
		ModelReference modelr = new ModelReference();

		int vertex_start = vertex_list.size();

		vertex_list.addAll(modelData.getVertexData());
		normal_list.addAll(modelData.getNormalData());
		colors_list.addAll(modelData.getColorData());

		int vertex_count = modelData.getVertexData().size();
		modelr.vertices = vertex_count;
		...
		modelFrames.addFrame("idle", 0, 0, modelr);

		modelData.setModelFrames(modelFrames);
	}
